...
const Slider = () => {
    ...
    console.log("=====", images);
    const [currentIndex,
        setCurrenIndex] = useState(0);
    const [active,
        setActive] = useState("active");
    const [startSlider,
        setStartSlider] = useState(0);
    const [endSlider,
        setEndSlider] = useState(5);
        ...
        <Wrapper>
            <SliderWrapper>
                ...
                <div className="control">
                    <button
                        className={`btn btn-prev ${currentIndex === 0
                        ? "disable"
                        : ""}`}
                        onClick={handlePrev}
                    ...
                    <button
                        className={`btn btn-next ${currentIndex === images.length - 1
                        ? "disable"
                        : ""}`}
                        onClick={handleNext}
                ...
                {isOpen && (
                    <div className="modal">
                        ...
                        <img src={imageSelect
                            ?.photos} alt="Full Size"/>
                    </div>
                )}
            </SliderWrapper>
            <div className="list-dot">
                {listSlider.map((item, index) => (<img
                    key={item.id}
                    src={item.photos}
                    alt=""
                    className={`dot ${item.id === currentIndex + 1
                    ? "active"
                    : ""}`}
                    onClick={() => handleSelectImage(item.id - 1)}
                    style={{
                    width: `${ (1 / listSlider.length) * 100}%`
                }}/>))}
            </div>
        </Wrapper>
    );
